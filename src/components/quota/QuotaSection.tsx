...
export function QuotaSection<TState extends QuotaStatusState, TData>({
  ...
  const handleRefreshItem = useCallback(
    async (file: AuthFileItem) => {
      ...
      setRefreshingItems((prev) => ({ ...prev, [file.name]: true }));
      try {
        await loadQuota([file], 'page', () => {});
      } finally {
          ...
          <div ref={gridRef} className={config.gridClassName}>
            {pageItems.map((item) => (
              <QuotaCard
                ...
                refreshing={refreshingItems[item.name] === true}
                refreshDisabled={disabled || loading || sectionLoading}
              />
            ))}
          </div>
